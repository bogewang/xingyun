...
- Added coverage for `syncConfirmAmount` writing `confirmAmt` back to the row while preserving `confirmNum` and `taxPrice`.
- Added `getConfirmAmount` empty `confirmNum`/`taxPrice` cases.
- Added `sumConfirmFields` empty-array and multi-row boundary cases.
- Focused Vitest result after this fix: `5` tests passed.
