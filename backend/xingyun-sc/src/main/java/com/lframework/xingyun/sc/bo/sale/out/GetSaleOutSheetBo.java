    ...
    public static class OrderDetailBo extends BaseBo<SaleOutSheetFullDto.SheetDetailDto> {
        ...
        protected void afterInit(SaleOutSheetFullDto.SheetDetailDto dto) {
            ...
            ProductUnit selectedUnit = product.getUnits() == null ? null : product.getUnits().stream()
                    .filter(item -> dto.getUnitId() != null && dto.getUnitId().equals(item.getId()))
                    .findFirst()
                    .orElse(baseUnit);
            ...
            this.costPrice = dto.getCostPrice() == null ? null
                    : dto.getCostPrice().multiply(unitRate).setScale(6, RoundingMode.HALF_UP);
            ...
            ProductStock productStock = productStockService.getByProductIdAndScId(this.getProductId(),
                    this.getScId());
            this.stockNum = productStock == null ? BigDecimal.ZERO :
                    productStock.getStockNum().divide(unitRate, 2, RoundingMode.HALF_UP);
