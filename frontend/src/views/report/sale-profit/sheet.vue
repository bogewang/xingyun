  ...
  <div v-permission="['report:sale-profit:query']">
    <page-wrapper content-full-height fixed-height>
      ...
      >
        ...
        <div v-show="!sheetChartEmpty" class="chart-grid">
          <div ref="sheetAmountPieChartRef" class="chart-container"></div>
          <div ref="sheetProfitPieChartRef" class="chart-container"></div>
        </div>
      </a-modal>
  ...
  export default defineComponent({
    ...
    methods: {
      ...
      renderCategoryPie() {
        ...
        const amountData = this.buildPieData(
          (detail) => detail.categoryName || '未分类',
          null,
          (detail) => this.calcDetailSalesAmount(detail),
        );
        const profitData = this.buildPieData(
          (detail) => detail.categoryName || '未分类',
          null,
          (detail) => this.calcDetailProfitAmount(detail),
        );
      ...
      renderProductPie(categoryName) {
        ...
        this.renderPieCharts(
          amountData,
          profitData,
          `${categoryName} - 商品销售额占比`,
          `${categoryName} - 商品销售利润占比`,
        );
      },
      buildPieData(getName, filterDetail, getValue) {
        const dataMap = {};
        (this.sheetChartDetails || [])
          .filter((detail) => !filterDetail || filterDetail(detail))
          .forEach((detail) => {
            const name = getName(detail);
            dataMap[name] = (dataMap[name] || 0) + Number(getValue?.(detail) || 0);
          });
      ...
      calcDetailProfitAmount(detail) {
        return Number(detail?.totalProfit || 0);
      },
      renderPieCharts(amountData, profitData, amountTitle, profitTitle, clickHandler) {
        this.sheetChartEmpty = amountData.length === 0 && profitData.length === 0;
        ...
        this.$nextTick(() => {
          this.sheetAmountPieChart?.resize();
          this.sheetProfitPieChart?.resize();
        });
      },
      renderPie(chartKey, chartRef, data, title, valueLabel, clickHandler) {
        ...
        if (!this[chartKey]) {
          this[chartKey] = echarts.init(chartDom);
        }
        this[chartKey].off('click');
        if (clickHandler && this.sheetChartLevel === 'category') {
          this[chartKey].on('click', clickHandler);
        }
        ...
        this[chartKey].setOption({
          ...
          tooltip: {
            trigger: 'item',
            formatter: (params) =>
              `${params.name}<br/>${valueLabel}：${this.formatCurrency(params.value)}<br/>占比：${
                params.percent
              }%`,
          },
          legend: {
