...
public class PurchaseOrderServiceImpl extends
    ...
    private static final List<DateTimeFormatter> IMPORT_DATE_FORMATTERS = Arrays.asList(
    ...
    private LocalDate parseImportOrderDate(String value, int rowIndex) {
        if (StringUtil.isBlank(value)) {
            throw new DefaultClientException("第" + rowIndex + "行“单据日期”不能为空");
        }
        ...
        for (DateTimeFormatter formatter : IMPORT_DATE_FORMATTERS) {
            try {
        ...
        throw new DefaultClientException("第" + rowIndex + "行“单据日期”格式错误");
    }

    private String defaultString(String value) {
        return Objects.toString(value, StringPool.EMPTY_STR).trim();
    }
}
