...
**Architecture:** 保留 `SaleOutSheetServiceImpl` 现有的出库单查询、商品批量加载、商品聚合和总计逻辑，将汇总键调整为“订单日期 + 商品 ID”。新增同包纯格式化器负责客户名称回退、日期格式化、客户明细字符串拼接和总重量格式化，服务层只负责按客户首次出现顺序组装格式化器输入并生成固定表头。
