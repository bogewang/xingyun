...
**Goal:** 将 `/sale/out` 买菜汇总导出中的客户动态列合并为一个“明细数量”列，将单位与总计合并为“总重量”列，并按日期 + 商品拆分汇总行。

**Architecture:** 保留 `SaleOutSheetServiceImpl` 现有的出库单查询、商品批量加载、商品聚合和总计逻辑，将汇总键调整为“订单日期 + 商品 ID”，并按品类、商品名称、日期排序。新增同包纯格式化器负责客户名称回退、日期格式化、客户明细字符串拼接和总重量格式化，服务层只负责按客户首次出现顺序组装格式化器输入并生成固定表头。
...
规则：昵称使用 `StringUtils.isNotBlank` 判断；日期使用 `yyyy/M/d` 格式；数量去除无意义尾零；备注使用 `LinkedHashSet` 去重并以 `；` 连接；数量非零时输出 `数量/单位`；数量为零但存在备注时仅输出客户和备注；客户明细为空时不参与连接；总重量输出 `数量单位`，单位为空时仅输出数量。
...
- Produces: 固定表头 `日期、商品名称、分类名称、总重量、明细数量`，单行内按客户首次出现顺序输出客户明细。

- [ ] **Step 1: Write/extend a failing regression assertion for the fixed header shape**

将表头构造抽成包可见静态方法 `buildMarketBuySummaryHeaders()`，先在测试中断言返回键顺序为 `date、productName、category、total、detail`，运行测试确认当前代码不存在该方法或行为不满足。
...
将每个日期和商品转换为独立 `SummaryRow`，将客户单元格转换为格式化器的 `CustomerDetail` 并把结果写入 `detail`；使用 `formatOrderDate(row.orderDate)` 输出日期，使用 `formatTotalWithUnit(row.total, row.unit)` 把数量 `6` 和单位 `公斤` 输出为 `6公斤`，删除按客户循环写列和独立单位列的逻辑；`SummaryRow.cells` 仍按客户 ID 聚合数量和去重备注。空单据或空明细也输出固定五列表头。
